...
const getStyleId = (category: string, title: string, description: string): string => {
  ...
  if (text.includes('dining') || text.includes('food') || text.includes('restaurant') || text.includes('מסעדה') || text.includes('אוכל')) return 'icon-dining';
  if (text.includes('landmark') || text.includes('sight') || text.includes('attraction') || text.includes('אתר') || text.includes('אטרקציה') || text.includes('history')) return 'icon-camera';
  if (text.includes('nature') || text.includes('park') || text.includes('garden') || text.includes('פארק') || text.includes('טבע')) return 'icon-parks';
  if (text.includes('art') || text.includes('museum') || text.includes('gallery') || text.includes('אמנות') || text.includes('מוזיאון')) return 'icon-arts';
  if (text.includes('shop') || text.includes('market') || text.includes('קניות') || text.includes('שוק')) return 'icon-shopping';
  if (text.includes('nightlife') || text.includes('bar') || text.includes('club') || text.includes('בילוי') || text.includes('בר')) return 'icon-bars';
...
export const generateKml = (data: TripData): string => {
  ...
  const planSuffix = isHe ? "תוכנית טיול" : "Trip Plan";
  const desc = isHe ? "שכבות טיול מותאמות אישית שנוצרו על ידי MapLayer AI." : "Personalized trip layers generated by MapLayer AI.";
